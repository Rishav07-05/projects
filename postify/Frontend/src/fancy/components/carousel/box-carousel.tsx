...
const CubeFace = memo(
  ({ transform, className, children, style }: FaceProps) => (
    <div
      className={cn(
...
const BoxCarousel = forwardRef<BoxCarouselRef, BoxCarouselProps>(
  (
   ...
   const _transition = useMemo(() => {
    ...
    }, [items.length, direction, currentRotation , _transition, handleAnimationComplete, baseRotateX, baseRotateY , currentItemIndex]);  
    ...
    }, [items.length, direction , _transition, handleAnimationComplete, baseRotateX, baseRotateY, currentRotation, currentItemIndex]);
